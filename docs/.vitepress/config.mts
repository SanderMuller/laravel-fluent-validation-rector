...
const site = 'https://sandermuller.github.io/laravel-fluent-validation-rector/'

/**
 * A markdown link between source pages (`05-converters.md#anchor`) points at a file that only exists
...
const description = 'Rector rules for migrating Laravel validation to sandermuller/laravel-fluent-validation: pipe strings, rule arrays, Rule:: objects and Livewire attributes all convert to FluentRule chains.'

export default defineConfig({
    title: 'Fluent Validation Rector',
    description,
    base: '/laravel-fluent-validation-rector/',
    cleanUrls: true,
    lastUpdated: true,
    ...
    head: [
        ...
        ['meta', { property: 'og:title', content: 'Laravel Fluent Validation Rector' }],
    ...
    buildEnd: async ({ outDir, srcDir }) => {
        ...
        try {
            ...
            intro = ['# Laravel Fluent Validation Rector', '', `> ${description}`].join('\n')
        }

        const index: string[] = [intro, '', '## Formats', '',
            `- [llms-full.txt](${site}llms-full.txt): every page below, in reading order, in one fetch`,
            `- Any page is also served as markdown at its own URL plus \`.md\` (for example [converters.md](${site}converters.md))`,
            '']
        ...
        const parts: string[] = ['# Laravel Fluent Validation Rector', '', `> ${description} Full documentation, ${pages.length} pages, in reading order. Index: ${site}llms.txt`, '']
    ...
    themeConfig: {
        nav: [
            ...
            { text: 'Configuration', link: link('12-configuration') },
            { text: 'Releases', link: 'https://github.com/SanderMuller/laravel-fluent-validation-rector/releases' },
            { text: 'Packagist', link: 'https://packagist.org/packages/sandermuller/laravel-fluent-validation-rector' },
        ],
        ...
        socialLinks: [
            { icon: 'github', link: 'https://github.com/SanderMuller/laravel-fluent-validation-rector' },
        ],

        editLink: {
            pattern: 'https://github.com/SanderMuller/laravel-fluent-validation-rector/edit/main/docs/:path',
