...
<details>
<summary>Conditional tuples and dynamic-arg handling</summary>
...
- **Dynamic concat rule strings**: an array element built by concatenation (`'required_if_accepted:' . $field`) lowers to the native fluent method when the rule name is a static leading literal and the rule takes the post-colon tail as a single string (`->requiredIfAccepted($field)`). It stays on a string-coercion-safe `->rule(<concat>)` escape hatch otherwise — when the tail is a single non-literal operand (the `.` coercion can't be proven), or for the value-appending `*If`/`*Unless` family where a single whole-tail arg would emit a spurious trailing comma.
...
- **Notes**:
  ...
  - A `FluentRule::field()->…->rule(Rule::array())` (or `->rule('array')`) parent is promoted to `array()` before folding — the array factory seeds the same implicit `array` rule, but only `array()` exposes `each()`. Gated to provably behavior-preserving chains: the `field()` root has no label arg, the only rule hop is the bare array rule, and every other hop is a presence/dependency modifier native to both `FieldRule` and `ArrayRule`. Labeled parents, `FieldRule`-only methods, conditionable closures, object-valued `->rule(...)` hops, a `message()` bound to the array rule, keyed `Rule::array([...])`, and macro-based size constraints keep the escape hatch.
...
<details>
<summary>Rewrite table, conditional-tuple handling, receiver inference</summary>
...
- **COMMA_SEPARATED conditional rules** (array, string, and concat forms): `->rule(['required_if', 'field', 'value'])`, `->rule('required_if:field,value')`, and `->rule('required_with:' . self::FIELD)` all lower to native methods (`->requiredIf('field', 'value')` / `->requiredWith(self::FIELD)`). Covers field-plus-variadic-values rules (`required_if` / `exclude_unless`) and pure variadic-fields rules (`required_with` / `prohibits` / `required_array_keys`). String form splits the tail on commas (arity ≥ 1 for pure-field, ≥ 2 for field-values); the fluent variadic re-joins with `,`, so escaped/quoted commas round-trip to the identical rule string. Concat form is scoped to the pure-field family — it passes the whole post-colon expression as a single field arg (provably equivalent via that same re-join) and gates to statically-simple, string-oriented operands (string literals, variables, class constants, enum `->value`); a method call / arithmetic / ternary operand stays an escape hatch. BackedEnum cases in array tail positions auto-wrap with `->value`. Category C `required_if_accepted` and Category D `exclude_with` stay as escape hatch.
...
Narrows the `@return` PHPDoc annotation on `rules()` methods from the wide `array<string, ValidationRule|string|array<mixed>>` union down to `array<string, \SanderMuller\FluentValidation\Contracts\FluentRuleContract>` when every value in the returned array is a `FluentRule::*()` call chain. Cosmetic (runtime behavior untouched), but gives PHPStan and editors a narrower type to reason about. Run as a separate pass after `CONVERT` stabilizes.

<details>
<summary>Qualifying classes, narrow conditions, skip conditions</summary>
...
- **No-ops on an older installed package.** The `schema()` builder and its runtime dispatch shipped in fluent-validation 1.31. A reflection-time surface probe (does the `FluentSchema` class exist?) makes the whole rule emit zero rewrites on an older install, where a converted `schema()` would never be dispatched (`createDefaultValidator` still calls `rules()`) and validation would silently break. The composer floor is `^1.32`; this guards a path/dev install that bypasses the constraint.
...
- **Rewrites self-referential `rules()` calls.** A `parent::rules()` in the body becomes `parent::schema($rules)` — when the parent provably converts (a concrete, or `#[FluentRules]`-opted, `HasFluentRules` class with a public `rules()`, resolved by reflection) **or already declares a `schema(FluentSchema)` builder** (converted in an earlier pass — so a re-run over a partly-converted chain finishes the child instead of leaving it stranded). A base that stays `rules()` (abstract-not-opted, vendor) leaves the child unconverted so the call keeps resolving. `$this->rules()` / `self::rules()` / `static::rules()` in the body are rewritten to the `schema($rules)` form too. Sibling parent calls (`parent::messages()`) are never touched.
- **Captures the builder into closures.** A `FluentRule::` chain (or a `parent::rules()` call) built inside a plain `function () { … }` closure — e.g. a `->when(…, function (RuleSet $rules) { return $rules->put('x', FluentRule::integer()…); })` callback — converts: the receiver is swapped onto the injected builder and the closure gains a `use ($builder)` capture so it's in scope. The builder is named to avoid clashing with the closure's own parameters (falling back from `$rules` to `$schema`, …). Arrow functions auto-capture and need no `use`.
...
- **Bails on**:
  ...
  - A `rules()` calling `parent::rules()` whose parent won't provably convert to `schema()` (abstract without `#[FluentRules]`, no trait, or an unresolvable/vendor base). Add `#[FluentRules]` to the `rules()` method to force the rewrite if you've audited the chain.
  - A self-referential `rules()` call (`$this->`/`self::`/`static::`) in a method other than `rules()` — the rename would strand it with no builder in scope.
  - A FluentRule chain (or a self/parent `rules()` call) built inside a scope the injected builder can't be threaded into — an anonymous class or a nested named function. (Plain closures convert via an injected `use ($builder)`; arrow functions auto-capture.) When such an un-reachable chain sits alongside a `parent::rules()` call, the bail is skip-logged rather than silent, since leaving it as `rules()` would strand once the base converts.
...
<details>
<summary>Scoping rules + what `#[FluentRules]` does NOT lift</summary>
...
<details>
<summary>Log file location, format, and parallel-worker rationale</summary>
...
- **`SCHEMA` inheritance chains must be processed together.** `ConvertToFluentSchemaRector` rewrites a child's `parent::rules()` to `parent::schema($rules)` when the parent *provably converts* (resolved by reflection). "Provably converts" means "would convert if processed" — Rector exposes no run-file-set to a rule, so it can't confirm the parent's file is in *this* run. Running `SCHEMA` on a lone child while excluding its (convertible, not-yet-`schema()`) parent — e.g. `rector process app/Http/Requests/ChildRequest.php` — rewrites the child but leaves the parent on `rules()`, producing a loud `Call to undefined method parent::schema()` (caught immediately by PHPStan / first request, never silent). Process the inheritance chain together — the directory / whole-codebase run the separate-`SCHEMA`-pass workflow already prescribes — or mark the child's `rules()` with `#[FluentRules]` only after converting the parent.
