...
- **Rewrites self-referential `rules()` calls.** A `parent::rules()` in the body becomes `parent::schema($rules)` — but only when the parent itself provably converts (a concrete, or `#[FluentRules]`-opted, `HasFluentRules` class with a public `rules()`, resolved by reflection); a base that stays `rules()` (abstract-not-opted, vendor) leaves the child unconverted so the call keeps resolving. `$this->rules()` / `self::rules()` / `static::rules()` in the body are rewritten to the `schema($rules)` form too. Sibling parent calls (`parent::messages()`) are never touched.
...
- **Bails on**:
  ...
  - A FluentRule chain (or a self/parent `rules()` call) built inside a scope the injected builder can't reach — a plain `function () { … }` closure, an anonymous class, or a nested named function (arrow functions auto-capture, so they stay convertible).
