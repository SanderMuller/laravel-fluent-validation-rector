...
* All 11 rules now short-circuit at `refactor()` entry on files that contain none of the rule-bearing tokens (`function rules`, `validate(`, `Validator::`, `RuleSet::`, `FluentRule`, etc.). Decision cached per file path. No public API change, no behavior change, no skip-log shape change.

**Full Changelog**: https://github.com/SanderMuller/laravel-fluent-validation-rector/compare/1.2.1...1.2.2
