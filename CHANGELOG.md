...
`ConvertLivewireRuleAttributeRector` now detects and warns on a silent
cross-package compose conflict: a Livewire component carrying a
property-level `#[Rule]` / `#[Validate]` attribute whose **abstract
ancestor** uses the `HasFluentValidation` trait (or its Filament
sister, `HasFluentValidationForFilament`).

In that shape Livewire's `validateOnly($field)` returns `[]` for the
attributed property at runtime, because the trait's `getRules()` reads
only the class's `rules(): array` method and does not inspect Livewire
attribute metadata. Worse, blind rector-side conversion would generate
a child `rules()` method that overrides the parent's — silently
dropping parent-owned fields. Production dogfood surfaced this on a
2FA flow where `validateOnly('phoneNumber')` silently no-op'd.

Pre-1.2.0 the rector either rewrote the attribute (clobbering the
parent's `rules()`) or fell through some other safety gate without
surfacing a diagnostic. Either way the silent-ignore was invisible.

Post-1.2.0 the rector bails conversion on this inherited-trait shape
and emits a verbose-mode skip-log entry naming the offending property
and listing two remediation paths:

- Move the rule into the parent's `rules(): array` method (so
  `getRules()` picks it up), **or**
- Drop the trait from the parent if Livewire-attribute validation was
  the intended pathway.

**Direct-trait use on the class itself is intentionally NOT flagged.**
That shape converts cleanly: the rector either merges the attribute
rule into a local `rules()` array, or installs one — no clobber risk,
no silent-ignore post-convert. (Pre-tag Codex adversarial review caught
the original detection scope flagging this convertible shape too; the
narrowing to ancestor-only is the result.)

### Why this matters

A Livewire `#[Rule]` attribute that silently returns `[]` is the worst
flavor of bug: the source reads as if validation is happening, the
validator runs without throwing, and the failing input slips through.
There is no log line, no thrown exception, and `php artisan livewire:lint` has no opinion on cross-package trait composition.

Layer-2 skip-log warnings exist for exactly this category — shapes where
the rector cannot safely transform AND the runtime behavior is silently
broken in a way the user is unlikely to notice without a diagnostic.
This is the second such warning to ship (the first being the parent-
final-rules guard from 0.16.0).

**Full Changelog**: https://github.com/SanderMuller/laravel-fluent-validation-rector/compare/1.1.0...1.2.0

## 1.1.0 - 2026-04-29

### RuleSet::from descent for converter rectors

#### What changed

##### Converter rectors descend into `RuleSet::from([...])`

Both `ValidationStringToFluentRuleRector` and
`ValidationArrayToFluentRuleRector` gained a new dispatch branch
that visits any `StaticCall(RuleSet, from, [Array_])` regardless of
surrounding-class qualification. The static-call signature
self-gates against false positives — `RuleSet::from(...)` is a
sister-package call that consumers don't write by accident.

Three contexts now covered:

```php
// FormRequest with `rules(): RuleSet` return — the canonical
// sister-package FormRequest pattern
final class StorePostRequest extends FormRequest
{
    public function rules(): RuleSet
    {
        return RuleSet::from([
            'name' => 'required|string|max:255',     // converts ✓
            'email' => 'required|email',             // converts ✓
        ]);
    }
}

// Action method — inline validation outside FormRequest. Pre-1.1.0
// silently skipped; post-1.1.0 the inner array converts.
final class AttachUserToCompanyAction
{
    public function execute(array $data): array
    {
        $safe = RuleSet::from([
            'role' => ['required', Rule::enum(CompanyRoleEnum::class)],
            'userId' => ['bail', 'required', Rule::numeric()->integer()],
        ])->check($data)->safe();
...
// Service-class method body — same broad-scope path as actions.
...
The `RuleSet::from([...])` wrapper static-call + any chained methods
(`->check(...)->safe()`) are sister-package API contract. The rector
replaces only the inner Array_ argument; the wrapper structure stays
byte-identical. Consumer-visible diff is only the rule shape inside
the array changing from string/array form to FluentRule chains.

##### Skip-log emit on non-literal argument

`RuleSet::from($injected)` (Variable arg, not a literal Array_) is
not statically determinable for descent. The rector emits a skip-log
entry with the same wording as `GroupWildcardRulesToEachRector`'s
existing skip on the same shape:
...
```
One consistent skip-reason text across all three rectors that touch
the wrapper. Consumers learn one wording, not three.

**Full Changelog**: https://github.com/SanderMuller/laravel-fluent-validation-rector/compare/1.0.0...1.1.0

## 1.0.0 - 2026-04-29

The package is now stable. The public API surface enumerated in
[`PUBLIC_API.md`](PUBLIC_API.md) is committed under [SemVer 2.0](https://semver.org).
Renames, removals, or value-vocabulary changes to anything listed there
are MAJOR-bump-only events; symbols not listed are `@internal` and may
change in any release without a MAJOR bump.

This is a tag-only release — there is no behavior change between 0.22.3
and 1.0.0. The same SHA carries both tags; the tag IS the commitment.

### What's frozen

- **12 rector class FQNs** under `SanderMuller\FluentValidationRector\Rector\`
- **6 set list constants** on `FluentValidationSetList` (`ALL`, `CONVERT`,
  `GROUP`, `TRAITS`, `POLISH`, `SIMPLIFY`)
- **Configuration constant names + their string wire keys** on the
  configurable rectors (constants and literal-string keys both protected)
- **Typed-config DTO builders** under `Config\` and shared value objects
  under `Config\Shared\` (`AllowlistedFactories`, `BaseClassRegistry`,
  `OverlapBehavior`)
- **Verbose-mode env var** `FLUENT_VALIDATION_RECTOR_VERBOSE` — name +
  canonical accepted values (`off`, `actionable`, `all`)
- **Skip-log file paths**, line format, and header shape (slot
  semantics, prefix, parenthesization)
- **Trait FQNs** referenced by the trait-add rectors

Heuristic boundaries — detection rules, classification confidence,
trace depth, skip-text reason content, fixture coverage, diagnostic
granularity — are explicitly NOT SemVer-committed. Heuristic
tightening continues to ship in MINOR releases.

### Compatibility

- **PHP**: 8.3, 8.4
- **Laravel**: 11, 12, 13
- **`sandermuller/laravel-fluent-validation`**: ^1.20
- **Rector**: 2.x

The 24-cell CI matrix (2 OS × 2 PHP × 2 stability × 3 Laravel) is green
on this release. 676 tests / 1261 assertions / 0 failures locally;
PHPStan, Pint, Rector all clean.

### Upgrading from 0.x

Existing `rector.php` files keep working unchanged. The 0.20 → 0.22
arc landed all SemVer-affecting cleanups:

- **Pre-0.20.0**: deprecation shims at the root namespace for
  `Diagnostics` and `RunSummary` (always `@internal`; rarely imported).
  Removed in 0.22.0. If your code imports the root-namespace forms,
  update to `Internal\Diagnostics` / `Internal\RunSummary` (or, better,
  stop importing them — they're internal).
- **Cross-rector configuration**: when configuring both
  `SimplifyRuleWrappersRector` and `UpdateRulesReturnTypeDocblockRector`
  with a custom allowlist, build a single `AllowlistedFactories`
  instance and feed both rectors. See README §"Cross-rector shared
  DTOs" for the canonical pattern.

**Full Changelog**: https://github.com/SanderMuller/laravel-fluent-validation-rector/compare/0.22.3...1.0.0
