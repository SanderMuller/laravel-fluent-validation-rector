...
- **Coordinated release with `laravel-fluent-validation` 1.19.0.** Phase 1-3 of `specs/fluent-validation-1-19-surface-support.md` shipped against the `^1.17` floor (works today against vendored 1.17.x except the sign-helper transform pin). Two follow-ups land when peer `91mf9wnj` cuts the upstream 1.19.0 tag: (a) bump `composer.json` to `^1.19`, (b) flip `tests/SimplifyRuleWrappers/Fixture/sign_helpers_on_numeric.php.inc` from no-change pin to transform fixture (reflection-based method-availability check will then resolve `->positive()` etc. on `NumericRule`). Coordination commit + RELEASE_NOTES_<next>.md draft happen post-tag.
...
- **`specs/validation-parity-tool.md`** — per-FormRequest harness that runs Laravel validation against pre-rector and post-rector rule shapes with fixture payloads and diffs the resulting error bags. Catches attribute-label and message-key drift that a rector's functional test suite misses: converted rules may produce the same pass/fail outcome but different `:attribute` substitution or different message-key paths, which is a user-visible regression. Main-package has no pre-rector notion, so the harness lives rector-side next to the codemods that caused the drift. Handoff from laravel-fluent-validation peer (wr018nqu, 2026-04-20).
- **`specs/fluent-validation-1-19-surface-support.md`** — main-package 1.19.0 (coordinated tag, not yet cut) adds 11 new `FluentRule::*` shortcut factories (ipv4 / ipv6 / macAddress / json / timezone / hexColor / activeUrl / regex / list / enum / declined), a new `DeclinedRule` class, and 4 `NumericRule` sign helpers (positive / negative / nonNegative / nonPositive). Three rectors grow: `SimplifyFluentRuleRector` gains factory-shortcut collapse entries, the converter rectors gain token-to-factory mappings (`'ipv4'` → `FluentRule::ipv4()`), and `SimplifyRuleWrappersRector` gains `enum` as a rewrite target plus literal-zero `gt`/`gte`/`lt`/`lte` → sign-helper mapping. Single spec because all three rectors share the same 1.19.0 surface delta. Handoff from laravel-fluent-validation peer (91mf9wnj, 2026-04-21).
- **`specs/skip-log-coverage-hardening.md`** — `GroupWildcardRulesToEachRector` imports the trait but has 0 `logSkip()` calls across 18 `return null` paths; users running the `GROUP` set on unsupported wildcard shapes see silent no-ops. Catalogue the decision points vs early-exits and emit short reasons at the former. The converter rectors (`ValidationString/ArrayToFluentRuleRector`) inherit logging via the `ConvertsValidationRuleStrings` trait-composition chain and are audit-only (not expected to need new emit sites unless the audit surfaces a concrete user-misleading bail). Handoff from laravel-fluent-validation peer (wr018nqu, 2026-04-20).
