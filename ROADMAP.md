...
- **`specs/fluent-validation-1-19-surface-support.md`** — main-package 1.19.0 (shipped as 1.23.0, 2026-04-23) adds 11 new `FluentRule::*` shortcut factories (ipv4 / ipv6 / macAddress / json / timezone / hexColor / activeUrl / regex / list / enum / declined), a new `DeclinedRule` class, and 4 `NumericRule` sign helpers (positive / negative / nonNegative / nonPositive). Three rectors grow: `SimplifyFluentRuleRector` gains factory-shortcut collapse entries, the converter rectors gain token-to-factory mappings (`'ipv4'` → `FluentRule::ipv4()`), and `SimplifyRuleWrappersRector` gains `enum` as a rewrite target plus literal-zero `gt`/`gte`/`lt`/`lte` → sign-helper mapping. Single spec because all three rectors share the same surface delta. Handoff from laravel-fluent-validation peer (91mf9wnj, 2026-04-21; re-confirmed 1.23.0 ship 2026-04-23).
- **`required_array_keys` → `requiredArrayKeys` tuple rewrite.** Small gap in `SimplifyRuleWrappersRector::RULE_NAME_TO_METHOD` / `V1_REWRITE_TARGETS` — the other 10 conditional/presence tuple heads already rewrite (see `tests/SimplifyRuleWrappers/Fixture/required_if_array_form_category_a.php.inc` + `required_with_array_form_category_b.php.inc`). `ArrayRule::requiredArrayKeys(string ...$keys)` ships in main-package, but the rector map is missing the entry. 2-line fix + fixture. Surfaced by laravel-fluent-validation peer (91mf9wnj, 2026-04-23) against hihaho codebase.
