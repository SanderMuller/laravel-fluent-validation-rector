...
- **Semantic-divergence skip-message enrichment.** `SimplifyRuleWrappersRector`'s `<method>() not on FieldRule` bail is generic. For methods in `PromoteFieldFactoryRector`'s `SEMANTICALLY_DIVERGENT_PROMOTION` blocklist (`accepted`, `declined` today) the consumer has no way to infer that auto-promotion was considered and blocked for a specific reason. Extend the existing polymorphic-verb hint pattern (`POLYMORPHIC_TYPED_VERBS` → `"… min() is type-dependent; consider FluentRule::string() / numeric() / …"`) with a `SEMANTIC_DIVERGENCE_HINTS` map emitting `"accepted() not on FieldRule — auto-promotion to FluentRule::boolean() blocked because boolean's implicit constraint rejects 'yes'/'on'/'true' which the accepted Laravel rule permits. Keep the field()->rule('accepted') escape hatch, OR explicitly use FluentRule::boolean()->accepted() if your form submits only 1/true/\"1\"."`. Pure UX — the blocklist itself is the safety win; this is the diagnostic polish that explains it. Surfaced in 0.13.1 dogfood.
